...
description: Use when the user asks to "build the app", "compile the project", "fix build errors", "build failed", "resolve compiler errors", "make it compile", "clean build", or when Swift/SwiftUI compilation errors need to be diagnosed and fixed. Also activates when working on iOS/macOS code after making changes that need to be verified via a build.
metadata:
  version: "1.0.0"
---
...
Succeed only after the selected scheme builds with no compiler errors. Stop and report a blocker when any of these conditions applies:

- The same error remains after three targeted fixes.
- The error depends on missing provisioning, credentials, or an unavailable dependency.
- The failing source is generated or vendored and should not be edited.
- Five complete build-fix iterations have not produced a clean build.

When stopping, report the remaining diagnostics with file and line references and name the specific next action. Do not change generated files, vendored dependencies, or project architecture to force a green result. If a concurrency fix requires an isolation decision that the code does not answer, ask before choosing an actor or global actor.
