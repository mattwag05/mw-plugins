---
name: pippin-release
description: Cut a Pippin release by updating the version, changelog, tag, repository, and Homebrew tap, then verify both installations. Use for release, ship, or version-bump requests.
metadata:
  version: "1.4.0"
---
...
The generated Claude Code adapter for `pippin` uses the bare `pippin` command, so the shadowed version is what Claude Code spawns as the MCP server. Both installations must be current.
