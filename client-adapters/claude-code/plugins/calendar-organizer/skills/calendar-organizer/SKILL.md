...
This creates a venv at `.venv/` with required packages (openpyxl, ics, python-dateutil, pytz) on first run, and is instant on subsequent runs.
...
```bash
.venv/bin/python scripts/parse_excel.py "/path/to/file.xlsx"
```
...
Apply these rules to every extracted event. See `references/time-interpretation-rules.md` for the complete ruleset.
...
**On-call rules:** See `references/on-call-rules.md`
...
```bash
echo '<events_json>' | .venv/bin/python scripts/generate_ics.py --timezone "America/New_York" --output "/path/to/output.ics"
```
