...
No API keys are needed, only an active ASC session.
...
re-read, and repeat for up to 5 cycles or until the row count stabilizes.
...
mentioned only in the summary and are not turned into issues.
