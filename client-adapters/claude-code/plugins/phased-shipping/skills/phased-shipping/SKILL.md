...
description: >-
  Ship a multi-phase engineering plan as a sequence of stacked pull requests, with
  task tracking and background CI repair. Use for long-running work with explicit
  phases, several logical commits, or several pull requests that should progress
...
- **Depends on** which prior phase's branch this stacks on, if any (the git base rather than a conceptual dependency)
- **Beads** to file for it (one per intended commit)

**Verify each phase's premise against the actual code both during planning and when the phase starts.** Plans and their research can be subtly wrong: a feature the plan says to "add" may already exist in another mode, the named bottleneck may be outside the proposed fix, or the cited project may be wrong. When exploration contradicts the plan, **re-scope the phase to the real gap and file a follow-up bead for the deferred remainder** rather than building to the stale plan or silently expanding scope. A phase that ships the missing 20% plus a clear bead for the rest beats one that rebuilds the existing 80%. Note the re-scope in the chat and the PR body.
...
The headers explain the design and its trade-offs rather than mechanically listing `## Summary` and `## Changes`. Each subsystem paragraph links the relevant files, records a rejected alternative, and points to the next pull request. The test plan reports deltas and names specific tests instead of giving counts alone. Adapt the structure to the actual change.
...
When you merge a stack bottom-up, **`gh pr merge <N> --delete-branch` can close the next pull request instead of retargeting it** if that request uses the deleted branch as its base. Auto-retarget can lose the race with branch deletion. Recovery requires recreating the deleted base ref, reopening the request, changing its base to `main`, and then merging.
...
- A git host with a pull-request or merge-request model and a CLI. Section 1 detects it. GitHub
  with `gh` also supports the native stacked-PR path in section 5. Forgejo, Gitea, GitLab, and
  other hosts use the manual `--base` flow.
